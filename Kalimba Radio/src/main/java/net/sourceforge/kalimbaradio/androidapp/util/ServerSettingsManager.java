    ...
    private void initDefault() {
        ...
        if (servers == null) {
            ...
            if (!oldServerSettingsFound) {
                int serverId = addServer("Kalimba Home",Constants.PREFERENCES_KEY_SERVER_ADDRESS, session.getUserDetails().get(SessionManager.KEY_SUBUSER), Constants.PREFERENCES_KEY_SERVER_USER_PASSWORD);
                setActiveServerId(serverId);
            }
        }
    }
